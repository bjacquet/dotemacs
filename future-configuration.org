...
** Display the current time in modeline

More and more often I'm running Emacs in fullscreen. I like to have a clock visible at all times and this displays one in the modeline.

#+begin_src emacs-lisp
(display-time-mode 1)
#+end_src

Apart from displaying time it also display load level and a mail flag—which I have never seen! I don't want to know about the load level.
...
#+begin_src emacs-lisp
(setq undo-tree-history-directory-alist '(("." . "~/.conf/emacs/cache/.undo_tree")))
#+end_src
...
(global-set-key (kbd "C-M-s-q") 'replace-string)

(global-set-key (kbd "C-M-s-u") 'undo-tree-visualize)


; (global-set-key (kbd "C-M-s-a") 'nil ) ; Reserved for Org-mode at work
