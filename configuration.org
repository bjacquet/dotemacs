...
** bj:insert-pull-request-link

Insert an org-mode link to the pull/merge request at URL.  The link description is built as ~REPO!NUMBER~.

   #+begin_src emacs-lisp
(defun bj:insert-pull-request-link (url)
  "Insert an org-mode link to the pull/merge request at URL.

The link description is built as REPO!NUMBER."
  (interactive "sURL: ")
  (let ((url-regexp
         "/\\([^/]+\\)/\\(?:-/\\)?\\(?:pull\\|pulls\\|pull-requests\\|merge_requests\\|merge-requests\\)/\\([0-9]+\\)")
        ;; Change this to use a different separator, e.g. "#" or " #".
        (separator "!"))
    (if (string-match url-regexp url)
        (let ((repo (match-string 1 url))
              (number (match-string 2 url)))
          (insert (format "[[%s][%s%s%s]]" url repo separator number)))
      (user-error "Not a valid pull/merge request URL: %s" url))))
   #+end_src
...
(defvar work-pc-p (zerop (or (string-match (system-name) "tama.local") 1)))
#+end_src
...
Selectrum is no longer maintained and their authors suggest using [[Vertico]] instead.
...
Required for Vertico to consider previously used commands, between Emacs restarts.
...
With [[https://github.com/KarimAziev/chrome-emacs][Chrome-Emacs]] installed in the browser I can edit any input field in Emacs.
...
[[https://emacs-lsp.github.io/lsp-mode/][lsp-mode]] is a client/library for the LSP.
...
[[https://github.com/emacs-lsp/lsp-ui][lsp-ui]] is a collection of UI modules for lsp-mode.
...
[[https://www.gnu.org/software/emacs/manual/html_node/eglot/index.html][Eglot]] is Emacs's client for the LSP.  The name is an acronym that stands for “Emacs Polyglot”.
...
The most advanced LSP for Elixir is [[https://expert-lsp.org/][Expert]].  It also takes some /expertise/ to
configure it correctly:
1. Download the binary from https://github.com/elixir-lang/expert/releases
2. Save it as ~~/.local/bin/expert~.
3. Make it executable ~chmod +x ~/.local/bin/expert~.

    #+begin_src emacs-lisp
(with-eval-after-load 'eglot
  (when (or work-pc-p plover)
    (let ((elixir-lsp-alternatives '(("~/.local/bin/expert" "--stdio") "elixir-ls")))
      (add-to-list 'eglot-server-programs
                   `((elixir-mode elixir-ts-mode) . ,(eglot-alternatives elixir-lsp-alternatives))))))
    #+end_src

**** Flymake

[[https://www.gnu.org/software/emacs/manual/html_node/flymake/index.html][Flymake]] is a syntax-checker and provides on-the-fly diagnostic annotations.
This defines key-bindings:

- ~C-c ! h~ Explain error at point.
...
[[https://emacs-tree-sitter.github.io/][tree-sitter]] is an Emacs binding for [[https://tree-sitter.github.io/tree-sitter/][Tree-sitter]], an incremental parsing system.

   #+begin_src emacs-lisp
(setq treesit-extra-load-path
      (list (expand-file-name (concat *emacs-dir* "tree-sitter"))))
   #+end_src
...
(setq treesit-language-source-alist
   ...
   To install any of these language grammars do ~M-x treesit-install-language-grammar~ and select it.
...
[[https://github.com/wkirschbaum/elixir-ts-mode][Elixir-TS-mode]] provides font-locking, navigation and indentation for Elixir using Treesitter.
...
(use-package elixir-ts-mode
  ...
  (add-hook 'elixir-ts-mode-hook
            (lambda ()
              (add-hook 'after-save-hook 'eglot-format nil 'make-it-local))))
    #+end_src
...
[[https://github.com/wkirschbaum/heex-ts-mode][Heex-ts-mode]] is major mode using Treesitter for fontification, navigation and indentation.
...
Install the LSP server, [[https://docs.astral.sh/ty/installation/#installing-globally-with-mise][ty]], with
...
The [[https://docs.astral.sh/ty/editors/#emacs][configuration]] for Eglot:
...
Install the LSP server, [[https://shopify.github.io/ruby-lsp/#with-other-editors][ruby-lsp]], with:
...
** Rust
   ...
   Install the LSP server, ~rust-analyzer~, with:
...
(defun bj:font-random ()
  ...
  (let* ((default-fonts (list "Anonymous Pro-16"
                              ...
                              "Victor Mono-15"
                              "iA Writer Mono S-15"))
         (fonts-work-pc-p (list "Anonymous Pro-16"
                                "Aporetic Serif Mono-14"
                                "CozetteVector-19"
                                "Departure Mono-14"
                                "Martian Mono-13"
                                ...
                                "Victor Mono-15"
                                "iA Writer Mono S-15"))
         (fonts (if work-pc-p fonts-work-pc-p default-fonts))
         font)
...
(when work-pc-p
  ...
  (global-set-key (kbd "C-s-a") 'bj:open-dashboard))
#+END_SRC
...
I also use Jira at work. =/

Setup the credentials.

   #+begin_src emacs-lisp
(when work-pc-p
  (bj:load-file *emacs-repo-dir* "secret/jira.el"))
   #+end_src

   [[https://github.com/nyyManni/jiralib2][Jiralib2]] provides many functions to interact with Jira's REST API.
...
*** bj:insert-org-jira-url

Given a ticket number returns its URL.
...
TICKET-NUMBER should be a string like \"fm 9081\"."
    ...
    (let* ((ticket-number (upcase (string-trim ticket-number)))
      ...
      (org-insert-link nil jira-url ticket-number))))
   #+end_src

*** bj:create-org-roam-ticket

    Given a ticket number creates an org file using the /ticket/ template.  Uses the
    ticket title for filename and ticket title, which fetches from Jira.
...
TICKET-NUMBER should be a string like \"fm 9081\"."
    ...
    (let* ((ticket-number (upcase (string-trim ticket-number)))
      ...
      (org-roam-node-find :initial-input node-title))))
   #+end_src

*** bj:slack-discuss-ticket

Given a ticket number creates a text message to start a ticket discussion in
Slack.
...
Given a ticket number creates a text message to request a pull request review in
Slack.
...
#+begin_src emacs-lisp
(and work-pc-p (bj:open-dashboard))
#+end_src
