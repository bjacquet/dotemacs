;;;-----------------------------------------------------------------------------
;;;
;;;           Copyright (C) 1996, 1997, 1998 by Rational Software.
;;;                           All rights reserved
;;;
;;;-----------------------------------------------------------------------------
;;;
...
;;;       ss-project-dirs '(("^Z:/siscog/" . "$/siscog/")))
;;;-----------------------------------------------------------------------------
;;; History
...
;;;					  SS-DO-COMMAND
;;;					Added definitions
;;;					  SS-VIEW-OTHER-VERSION
;;;					  SS-GET-IN-FILE
;;;					  SS-SHOW-FIND-IN-FILES
;;;					  SS-FIND
;;;					  SS-GET-FILE-VERSION
;;;					  SS-SHOW-HISTORY
;;;					  SS-SISCOG
...
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Deal with cmd "FINDINFILES"
;;;-----------------------------------------------------------------------------
	  ...
	  (t
	   ...
	   (let ((proj-file (ss-project-filename file)))
	     (cond ((stringp proj-file) )
		   ...
		   ((string-equal cmd "FINDINFILES") (setq proj-file file))
...
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Deal with writable files
;;;-----------------------------------------------------------------------------
(defun ss-verify-overwrite (f &optional forcep)
  (and (or (not (ss-file-modified-p f))
	  (ss-verify-test forcep
			  "%s differs from the checked-in file.  Replace your copy? " f))
      (or (not (file-writable-p f))
	  (ss-verify-test forcep
			  "%s is writable.  Replace your copy? " f))))
...
;;;SS-DIFF-WITH
;;;Description
;;;	Compare the current buffer to a historical version of the file under SourceSafe.
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-diff-with (compare-to-baseline-p non-interactive-p)
  (interactive (let* ((f (buffer-file-name))
		      (fbase (ss-baseline-name f)))
		 (list
		  (and (file-exists-p fbase)
  ...
  (message "Starting DIFF...")
  (ss-save-buffer)
  (let ((b (ss-diff-file-with (buffer-file-name) (not non-interactive-p))))
    (if (bufferp b)
	(progn
	  (pop-to-buffer b)
	  (goto-char (point-min))))))
...
;;;SS-DIFF-FILE-WITH
;;;Description
;;;	Diff the file F against a historical version in SourceSafe. 
;;;	(ss-get-file-version f)
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-diff-file-with (f interactivep)
  (cond (interactivep
	 ;; Use EDIFF to browse the diff
	 (let ((version (ss-get-file-version f)))
	   (when version
	     (let ((ss-file (ss-get-version f (format "v%s" version))))
	       (ss-do-ediff-files ss-file f `(ss-ediff-cleanup-fn ',ss-file))
	       ;; Return NIL so ss-diff knows that the diff has been done
	       nil))))
	...
	((eq (car ss-diff-program) 'DIFF)
	 (let ((version (ss-get-file-version f)))
	   (when version
	     (let ((ss-file (ss-get-version f (format "v%s" version))))
	       (ss-do-diff-files ss-file f (cdr ss-diff-program))))))
...
;;;SS-SISCOG
;;;Description
;;;	Gets the latest version for SISCOG
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-siscog ()
  (interactive)
  (message "Starting GET...")
  (cond ((ss-verify-test t "Really update Siscog?")
	 (ss-do-command "GET" "x:/win-nt/siscog/" nil "-R" "-W" "-I-N")
	 t)
	(t nil)))
...
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	If the file exists delete it first.
;;;-----------------------------------------------------------------------------
...
;;;SS-SHOW-HISTORY
;;;Description
;;;	Show a menu with all versions of a file to user select one
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-show-history (f)
  (let ((items nil)
	(tmpfile (ss-tmp-file "sshist2.txt")))
    (ss-do-command "HISTORY" f nil
		   (concat "-O@" (ss-external-filename tmpfile)))
    (if (file-exists-p tmpfile)
	(let ((buf (get-buffer-create "*SS GET FILE VERSIONS*"))
	      (last nil))
	  (set-buffer buf)
	  (erase-buffer)
	  (insert-file-contents tmpfile)
	  ...
	  (save-excursion
		...
		(next-space)
		(setf end (point))
		(setf last (car (read-from-string (buffer-substring start end)))))))
	  (while (re-search-forward "User: " nil t)
		...
		(previous-line 1)
		(beginning-of-line)
		(cond ((looking-at "Label: ")
		       (next-space)
		       (let ((start (+ (point) 2))
			     (end nil))
			 (end-of-line)
			 (setf end (- (point) 1))
			 (setf label (buffer-substring start end))))
		      ((looking-at "\*\*\*\*")
		       (when (re-search-forward "****  Version " nil t)
			 (let ((start (point))
			       (end nil))
			   (next-space)
			   (setf end (point))
			   (setf number (car (read-from-string (buffer-substring start end)))))))))
	      (save-excursion
		(beginning-of-line)
		(let ((start (point))
		      (end nil))
		  (end-of-line)
		  (setf end (point))
		  (setf date (buffer-substring start end))))
	      (if number
		  (push (sc-make-menu-item (format "%s - %s" number date) number) items)
		  (if label
		      (push (sc-make-menu-item (format "%s - %s" label date) last) items)))
	      (when number
		(setf last (- number 1)))))
	  (kill-buffer buf))
	(message "SS GET FILE VERSIONS failed."))
    (nreverse items)))
...
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-get-file-version (f)
  (let ((items (ss-show-history f)))
    (when items
      (sc-popup-menu t "Select File Version" items nil 40))))
...
(defun ss-find (name)
  ...
  (message "Looking for files containing %s (this may take a while)..." name)
  (let ((tmpfile (ss-tmp-file "ssfind.txt")))
    (ss-do-command "FINDINFILES" name nil "-R" "-IC"
		   (concat "-O@" (ss-external-filename tmpfile)))
    (if (file-exists-p tmpfile)
	(progn
	  (pop-to-buffer "*SS find*")
	  (erase-buffer)
	  (insert-file-contents tmpfile)
	  (goto-char (point-min))
	  (message "SS SEARCHING complete."))
      (message "SS SEARCHING failed."))))
...
;;;SS-SHOW-FIND-IN-FILES
;;;Description
;;;	Search for files that contains a given string and present a menu with them
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-show-find-in-files (name)
  (let ((items nil)
	(tmpfile (ss-tmp-file "ssfind2.txt")))
    (ss-do-command "FINDINFILES" name nil "-R" "-IC"
		   (concat "-O@" (ss-external-filename tmpfile)))
    (if (file-exists-p tmpfile)
	(let ((buf (get-buffer-create "*SS FIND IN FILES*")))
	  (set-buffer buf)
	  (erase-buffer)
	  (insert-file-contents tmpfile)
	  (beginning-of-buffer)
	  (while (re-search-forward "\$\/" nil t)
		...
		(end-of-line)
		(setf end (- (point) 1))
		(setf file (buffer-substring start end)))
	      (when file
		  (push (sc-make-menu-item (format "z:/%s" file) (format "z:/%s" file)) items))))
	  (kill-buffer buf))
	(message "SS SEARCHING failed."))
    (nreverse items)))
...
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-get-in-file (name)
  (interactive "sString to Search For: ")
  (message "Looking for files containing %s (this may take a while)..." name)
  (let ((items (ss-show-find-in-files name)))
    (if items
	(let ((file (sc-popup-menu t "Select File" items nil 40)))
	  (when file
...
;;;SS-VIEW-OTHER-VERSION
;;;Description
;;;	Presents a menu with all the versions of a given file
;;;History
;;;	Date		Author		Description
;;;	99/10/19	J. P. Varandas	Created
;;;-----------------------------------------------------------------------------
(defun ss-view-other-version ()
  (interactive)
  (message "Starting VIEW OTHER VERSION...")
  (let ((f (buffer-file-name))
	(bf (buffer-name))
	(tmpfile (ss-tmp-file "ssversion.txt")))
    (let* ((version (ss-get-file-version f)))
      (when version
	(ss-do-command "VIEW" f nil (format "-V%s" version)
		       (concat "-O@" (ss-external-filename tmpfile)))
	(if (file-exists-p tmpfile)
	    (progn
	      (pop-to-buffer (format "%s.\~%s\~" bf version))
	      (erase-buffer)
	      (insert-file-contents tmpfile)
	      (goto-char (point-min))
	      (message "SS VIEW OTHER VERSION complete."))
	    (message "SS VIEW OTHER VERSION failed."))))))
...
(defun ss-status ()
  ...
  (let ((f (if (eq major-mode 'dired-mode)
	       dired-directory
	       (buffer-file-name)))
	(tmpfile (ss-tmp-file "ssstatus.txt")))
    (ss-do-command "STATUS" f nil "-R"
		   (concat "-O@" (ss-external-filename tmpfile)))
    (if (file-exists-p tmpfile)
	(progn
...
(define-key ss-menu-map [ss-siscog]     '("Version to Siscog" . ss-siscog))
(define-key ss-menu-map [separator5]    '("--"))
(define-key ss-menu-map [ss-help]       '("Help" . ss-help))
(define-key ss-menu-map [ss-get-in-file ] '("Find in Files + Edit" . ss-get-in-file))
(define-key ss-menu-map [ss-find]       '("Find in Files" . ss-find))
(define-key ss-menu-map [ss-locate]     '("Find File" . ss-locate))
(define-key ss-menu-map [separator4]    '("--"))
(define-key ss-menu-map [ss-view-other-version] '("View Other Version" . ss-view-other-version))
(define-key ss-menu-map [ss-history]    '("Show History" . ss-history))
(define-key ss-menu-map [separator3]    '("--"))
(define-key ss-menu-map [ss-diff-with]  '("Show Differences With" . ss-diff-with))
