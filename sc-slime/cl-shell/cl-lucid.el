...
(define-key lisp-mode-map "\C-c>" 'cl-edit-callers)
(define-key cl-shell-mode-map "\C-c>" 'cl-edit-callers)
...
(put 'define-condition 'lisp-indent-function 1)
(put 'handler-bind 'lisp-indent-function 1)
(put 'handler-case 'lisp-indent-function 1)
(put 'restart-bind 'lisp-indent-function 1)
(put 'restart-case 'lisp-indent-function 1)
(put 'with-simple-restart 'lisp-indent-function 1)
...
(defun cl-get-function-name (&optional prompt)
  (let (f-name)
	...
	(error nil)))
    ;; AQUI
    (setq f-name (read-string (or prompt "Function name: ") (or f-name "")))
...
(defun cl-edit-definition (symbol)
  ...
  (cl-send-request cl-eval-stream-id
		   (cl-get-buffer-package)
		   (format "(let ((*print-length* nil)
                                  (*print-level* nil))
                              (format lisp:*standard-output*
                                      \"(cl-edit-multiple-definitions '~S)\"
                                      (lucid::get-cl-source-file %s)))"
			   symbol)))

(defun cl-edit-callers (symbol)
  (interactive (cl-get-function-name "Edit callers of: "))
  (cl-send-request cl-eval-stream-id
		   (cl-get-buffer-package)
		   (format "(let ((*print-length* nil)
                                  (*print-level* nil))
...
(defun cl-make-defstruct-regexp (symbol type-spec)
  (format "(defstruct[ \t\n]*(?%s[ \t\n]*" (cl-strip-package (cadr type-spec))))

(defun cl-definition-regexp (type-spec)
  (let ((symbol (second type-spec)))
...
(defun cl-edit-multiple-definitions (type-spec-and-file-list)
  (cond ((eq type-spec-and-file-list ':UNBOUND)
	 (message "Symbol has no function binding."))
	((or (null type-spec-and-file-list) (eq type-spec-and-file-list 'NIL))
	 (message "Not found"))
	((= (length type-spec-and-file-list) 1)
	 (cl-goto-definition (car type-spec-and-file-list)))
	(t (let ((user-choice (cl-choose-definition type-spec-and-file-list)))
	     (cond ((numberp user-choice)  ;user chose a definition to edit.
		    (setq cl-edit-next-definition-form
			  (list 'cl-edit-multiple-definitions type-spec-and-file-list))
		    (setq user-choice
			  (max 0 (min (1- (length type-spec-and-file-list))
				      user-choice)))
		    (cl-goto-definition (nth user-choice type-spec-and-file-list)))
		   ((eq user-choice 'edit-all) ;user wants to cycle through all
		    (cl-cycle-through-definitions type-spec-and-file-list))
		   (t (message "Cancelled.")))))))


;;; If user hits return, we just cycle through the definitions like
;;; the usual next-tag function.
(defun cl-cycle-through-definitions (type-spec-and-file-list)
  (let ((type-spec-and-file (car type-spec-and-file-list))
	(rest-of-list (cdr type-spec-and-file-list)))
    (setq cl-edit-next-definition-form
	  (if rest-of-list
	      (list 'cl-cycle-through-definitions rest-of-list)
	      nil))
    (cl-goto-definition type-spec-and-file)))

;;; Provide both standard and "electric" cursor movement keybindings.
(defvar cl-choose-definition-map
  (let ((map (make-keymap)))
    ;; AQUI
    (if (not (member 'xemacs features))
	(fillarray (if emacs-version-18-p map (car (cdr map)))
	       'cl-choose-definition-undefined))
    ...
    (define-key map "\C-m" '(lambda () (interactive)
			      (throw 'exit (1- (count-lines 1 (point))))))
...
(defun cl-choose-definition (type-spec-and-file-list)
  (save-window-excursion
    (let ((buf (get-buffer-create "*CL definitions*"))
      ...
      (erase-buffer)
      (insert (format "Select a definition to edit:\n"))
    ...
    (cond ((null filename)		;if defined at top-level.
	  ...
	  (t
	   (let* ((buf (find-file-noselect (expand-file-name filename)))
		 ...
		 (pop-to-buffer buf))
	     (goto-char (point-min))
	     (setq regexp (cl-definition-regexp type-spec))
	     (if (re-search-forward regexp nil t)
		 (goto-char (match-beginning 0))
		 (progn
		   (message "Can't find a definition for %s." type-spec)
		   ;; Quando falha porque o nome do metodo tem um caracter como o ? ...
;;;		   (when (re-search-forward (format "%s[ \t\n]*%s" (first type-spec) (second type-spec)) nil t)
;;;		     (goto-char (match-beginning 0)))
		   ))
